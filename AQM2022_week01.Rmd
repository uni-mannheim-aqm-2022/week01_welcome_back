...
+ Address open questions about the lecture. The lab will not be a second lecture, however, I am more than happy to answer your questions about the lecture--if I can. 
...
I aim split each lab session into two parts:
  ...
  - **Phase 2** (~30 minutes): Now it's your turn! In this phase you will work on exercises that are similar to the stuff discussed in phase 1. This is all about you trying things out, doing mistakes, trying again, and succeed. Within this phase we will distribute you into break out rooms. Think of it like virtual neighbors in the lab you can talk to. You can talk to your neighbors if you have any problems or, if things are easy for you, to explain the stuff you learned to others. I will be around as well and try to solve problems and difficulties that come up. Please do not hesitate at all to make use of that.
...
The homework assignments will be challenging. Some parts will potentially be very challenging. While there will be some parts that ask you to directly apply material from the lecture and the lab, there will be other exercises that will introduce new but related material. Since this is an elective, advanced course, I expect you to approach challenges with ambition and an open mind. Please work in groups (preferably groups of three). Plan at least one day--a.k.a. 8 hours--to work through all exercises. 
...
Homework assignments will be distributed via GitHub after the lecture on Wednesdays. Assignments are due by the following Tuesday, 23:59. 
...
You will hand in your homework assignments on GitHub. Your repository should include at least two files:
  ...
  + an `.Rmd` file with your solutions. The R-Code must replicate your solutions.
  + a `.pdf` write-up. This should just be the knitted version of your `.Rmd` file. Make sure to use chunk-option to format the write-up nicely. Avoid using machine-specific lines, so your code is reproducible. 
...
This is a challenging course in a challenging time. It is also an elective course, so we expect a high level of commitment towards the learning goals of the course. However, the pandemic puts a significant strain on all of us and, of course, also affects our performance capacity. For that reason, we would like to emphasize some points:
...
Now we want to multiply two matrices:
